...
for(let i = 0; i < 2; i++){
    ...
    console.log(verificaNumber(codigProd) )
    /*if(verificaNumber(codigProd) != false && verificaNumber(precoProd) != false && 
    verificaNumber(quantProd) != false) {
...
function verificaNumber(valor){
    let numero = typeof(valor) == 'number' ? valor : false
    return numero
}

let codigProd = readlineSync.question(`teste: `)
let numero = typeof Number(codigProd) == Number ? codigProd : false
